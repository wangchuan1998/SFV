...
function addFriend(userid){
	$.ajax({
			...
			type: 'POST',
			success:function(){
			    layer.msg('好友请求已经发送!', 2, -1);
			},
			error:function(){
			    layer.alert('请求发送失败，请稍候再试!', 2, -1);
			}
	});
}
			...
			success:function(json){
				...
				if(flag == "success"){
					...
					for(var i=0; i<data.length; i++){
						...
						var dt = $("<dt>");
						var a = "<a href='viewIndex.do?userid="+user.id+"'><img src='" + getRootPath() + user.headUrl + "'/></a>";
						dt.append(a);
						...
						var addFriend_a = "<a style='margin: 0 0 0 3px;' onclick='addFriend(\"" + user.id + "\");' class='m_dzh'>加好友</a>";
						
						dd.append(span1);
						dd.append(span2);
						dd.append(span3);
						dd.append(sendMsg_a);
						dd.append(addFriend_a);
